...
class DynamicModule:
    ...
    _statements: list[Statement] = dataclasses.field(default_factory=list)
    ...
    @property
    def statements(self) -> queries.QueryList[Statement]:
        return queries.QueryList(self._statements)
    ...
    def functions(self) -> queries.Query[Def]:
        return self.statements.instanceof(Def)

    @property
    def methods(self) -> queries.Query[Def]:
        return self.functions.filter(Def.is_method)

    @property
    def classes(self) -> queries.Query[Class]:
        return self.statements.instanceof(Class)

    @property
    def assigns(self) -> queries.Query[AnnAssign]:
        return self.statements.instanceof(AnnAssign)

    def get_class(self, name: Variable) -> typing.Optional[Class]:
        """Return the corresponding class if it exists, defaults to None."""
        return self.classes.filter(lambda cls: cls.name == name).first()

    def get_function(self, name: Variable) -> typing.Optional[Def]:
        """Return the corresponding class if it exists, defaults to None."""
        return self.functions.filter(lambda cls: cls.name == name).first()
