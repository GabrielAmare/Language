...
from website.language.base.models import Descriptor, get_shared_descriptor
...
class Node(graphs.Node):
    toplevel: bnf.TopLevel
    descriptor: Descriptor
    optimized: Descriptor

    @property
    def type(self) -> bnf.Variable:
        return self.toplevel.type

    @property
    def bases(self) -> list[bnf.Variable]:
    ...
    def is_direct_super_of(self, other: Node) -> bool:
        ...
        return isinstance(self.toplevel, bnf.Group) and other.toplevel.type in self.toplevel.types

    def order(self) -> tuple[int, bnf.Variable]:
...
class Graph(graphs.Graph[Node, Link]):
    reader: bnf.Reader = None

    def _new_node(self, toplevel: bnf.TopLevel, descriptor: Descriptor, **__) -> Node:
    ...
    def node(self, toplevel: bnf.TopLevel, descriptor: Descriptor) -> Node:
    ...
    def root_classes(self) -> queries.Query[bnf.Variable]:
    ...
    @classmethod
    def from_reader(cls, reader: bnf.Reader) -> Graph:
    ...
    def debug(self) -> None:
        ...
        for key in keys:
            ...
            if n_use > 0 and n_def == 0:
                website.console.warning(f"The type `{key!s}` is used {n_use!r} times, but not defined !")

            if n_def > 1:
                website.console.warning(f"The type `{key!s}` is defined {n_def!r} times !")

        root_classes = set(self.root_classes)
        if len(root_classes) > 1:
            website.console.warning(f"There are multiple root classes : {', '.join(map(str, root_classes))}")

        for node in self.nodes:
            if node.origin_order == 0:
                if isinstance(node.toplevel, (bnf.Branch, bnf.PatternGR)):
                    website.console.warning(f"{str(node.toplevel.type)!r} should have a super class.")