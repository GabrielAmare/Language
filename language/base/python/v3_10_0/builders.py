...
VARIABLE_LIKE = str | Variable
TYPE_LIKE = type | Expression | None


def _to_variable(value: VARIABLE_LIKE) -> Variable:
...
def _from_variable(value: VARIABLE_LIKE) -> str:
    if isinstance(value, Variable):
...
def _as_type(__type: TYPE_LIKE) -> Expression | None:
    if __type is None:
        return None
...
class Container(Context, abc.ABC):
    ...
    def CLASS(self, name: VARIABLE_LIKE) -> ClassBuilder:  # NOQA
        sub_ctx = ClassBuilder(root=self, name=name, _parts=[])
        self._parts.append(sub_ctx)
        return sub_ctx
    
    def FUNCTION(self, name: VARIABLE_LIKE) -> FunctionBuilder:  # NOQA
        sub_ctx = FunctionBuilder(root=self, name=name, _parts=[])
        self._parts.append(sub_ctx)
        return sub_ctx
    
    def SWITCH(self) -> SwitchBuilder:  # NOQA
        proxy = SwitchBuilder(root=self, _cases=[], _default=None)
    ...
    def WHILE(self, test: Expression) -> BlockBuilder:  # NOQA
        proxy = WhileBuilder(root=self, test=test, block_proxy=BlockBuilder(root=self, _parts=[]))
        self._parts.append(proxy)
        return proxy.block_proxy
    
    def METHOD(self, name: VARIABLE_LIKE) -> FunctionBuilder:  # NOQA
    ...
    def ASSIGN(self, name: VARIABLE_LIKE, type_: TYPE_LIKE = None, value: Expression | None = None) -> None:  # NOQA
        stmt = Assign(
            target=_to_variable(name),
            type=_as_type(type_),
            value=value
        )
        self.STATEMENT(stmt)
    
    def IADD(self, name: VARIABLE_LIKE, value: Expression | None = None) -> None:  # NOQA
        stmt = IAdd(
            target=_to_variable(name),
            value=value
        )
...
@dataclasses.dataclass
class SwitchBuilder(StatementBuilder, Contained, Context):
...
@dataclasses.dataclass
class WhileBuilder(StatementBuilder, Contained):
    test: Expression
    block_proxy: BlockBuilder
    
    def build(self) -> While:
        return While(
            test=self.test,
            block=build(self.block_proxy)
        )
...
class ParamBuilder(Builder[Param]):
    _name: VARIABLE_LIKE
    _type: TYPE_LIKE = None
    _default: Expression | None = None
    
    def type(self, __type: TYPE_LIKE) -> ParamBuilder:
        self._type = __type
        return self
    
    def build(self) -> Param:
        return Param(
            name=_to_variable(self._name),
            type=_as_type(self._type),
...
class FunctionBuilder(StatementBuilder, Contained, Container):
    ...
    _returns: TYPE_LIKE = None
    _decorators: list[Expression] = dataclasses.field(default_factory=list)
    
    def decorate(self, expr: Expression) -> None:
        self._decorators.append(expr)
    
    def param(self, name: VARIABLE_LIKE) -> ParamBuilder:
        proxy = ParamBuilder(name)
        self._params.append(proxy)
        return proxy
    
    def returns(self, expr: TYPE_LIKE) -> None:
        self._returns = expr
    ...
    def build(self) -> DecoratorGR:
        result = Function(
            ...
            returns=_as_type(self._returns),
        )
