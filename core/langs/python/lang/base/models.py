...
from base.processing import Element, Lemma, Token
...
class Except(CodeGR):
    ...
    def from_ast(cls, obj: ast.ExceptHandler) -> Except:
        return cls(error=Expression.__from_ast__(obj.type), as_=Variable.from_str(obj.name) if obj.name else None,
...
class ForIfClause(CodeGR):
    ...
    def from_ast(cls, obj: ast.comprehension) -> ForIfClause:
        return cls(target=StarTargetsGR.__from_ast__(obj.target), iter=Disjunction.__from_ast__(obj.iter),
...
class WithItem(CodeGR):
    ...
    def from_ast(cls, obj: ast.withitem) -> WithItem:
        return cls(context_expr=Expression.__from_ast__(obj.context_expr),
...
class Argument(DefArgumentGR):
    ...
    def from_ast(cls, obj: ast.arg, default=None) -> Argument:
        return cls(name=Variable.from_str(obj.arg),
...
class Elif(ElseGR):
    ...
    def from_ast(cls, obj: ast.If) -> Elif:
        return cls(condition=Expression.__from_ast__(obj.test), block=Block.from_ast(obj.body),
...
class Raise(Statement):
    ...
    def from_ast(cls, obj: ast.Raise) -> Raise:
        return cls(expr=Expression.__from_ast__(obj.exc),
...
class Slice(SliceGR):
    ...
    def from_ast(cls, obj: ast.Slice) -> Slice:
        return cls(lower=Expression.__from_ast__(obj.lower),
    ...
    def from_ast(cls, obj: typing.Union[ast.Assign, ast.AnnAssign]) -> AnnAssign:
        if isinstance(obj, ast.Assign):
            if len(obj.targets) == 1:
                return cls(target=Returnable.__from_ast__(obj.targets[0]), annotation=None,
                           value=Expression.__from_ast__(obj.value))
            else:
                raise NotImplementedError(ERR_9, obj)
        elif isinstance(obj, ast.AnnAssign):
            return cls(target=Primary.__from_ast__(obj.target), annotation=Expression.__from_ast__(obj.annotation),
...
class For(ScopeGR):
    ...
    def from_ast(cls, obj: ast.For) -> For:
        return cls(target=StarTargetsGR.__from_ast__(obj.target), iterator=Expression.__from_ast__(obj.iter),
                   block=Block.from_ast(obj.body), alt=Else.from_ast(obj.orelse) if obj.orelse else None)
...
class IfExp(Expression):
    ...
    def from_ast(cls, obj: ast.IfExp) -> IfExp:
        return cls(body=Disjunction.__from_ast__(obj.body), test=Disjunction.__from_ast__(obj.test),
...
class Try(ScopeGR):
    ...
    def from_ast(cls, obj: ast.Try) -> Try:
        return cls(block=Block.from_ast(obj.body),
...
class With(ScopeGR):
    ...
    def from_ast(cls, obj: ast.With) -> With:
        return cls(items=list(map(WithItem.from_ast, obj.items)), block=Block.from_ast(obj.body),
...
class Class(ClassGR):
    ...
    def from_ast(cls, obj: ast.ClassDef) -> Class:
        ...
        return cls(decorators=list(map(Decorator.from_ast, obj.decorator_list)) if obj.decorator_list else None,
                   name=Variable(obj.name), mro=bases,
...
class Def(ClassGR):
    ...
    def from_ast(cls, obj: ast.FunctionDef) -> Def:
        return cls(decorators=list(map(Decorator.from_ast, obj.decorator_list)) if obj.decorator_list else None,
                   name=Variable(obj.name), args=DefArgumentGR.from_ast_list(obj.args),
                   block=Block(statements=list(map(Statement.__from_ast__, obj.body))),
...
class DictComp(DataGR):
    ...
    def from_ast(cls, obj: ast.DictComp) -> DictComp:
        return cls(elt=DictItem.from_ast(obj.key, obj.value),
...
class Variable(AtomGR, StarTargetsGR):
    ...
    def as_def_arg(self, default: typing.Optional[Expression] = None,
                   type_: typing.Optional[Expression] = None) -> DefArgumentGR:
